...
/**
  ...
  먼저 매개변수 타입을 정의하고, 다음으로 반환값의 타입을 정의한다.
 */
...
function introduceB(name: number = "유혁스쿨") {
  console.log(`name : ${name}`);
}
...
/* 
tall이라는 매개변수를 생략하고 싶다면 매개변수 타입 선언 전 변수의 바로옆에 물음표?를 넣어  
마치 이전에 객체타입에 대해 살펴볼 때 선택적으로 프로퍼티를 만들었던 것과 같이 선택적 매개변수로 만들어 주면 된다.  
이렇게 선택적 매개변수가 된 tall이라는 매개변수에 마우스 커서를 올려보면 `(parameter) tall: number | undefined`와 같이  
tall 변수에 들어오는 값이 number 타입의 값이 들어올 수도 있거나 혹은 값이 아에 안들어올 수도 있기 때문에 number와 undefined의 유니온 타입으로 추론되는것을 확인할 수 있다.  
*/
...
*/
function introduceE(name = "유혁스쿨", tall?: number) {
  console.log(`name : ${name}`);
...
/* 
### 선택적 매개변수 - 타입 좁히기
이런 경우 조건문을 이용하여 타입 가드를 만들어 타입을 좁혀줘야 한다.  
*/
